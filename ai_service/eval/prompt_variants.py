...
用法（在 ai_service 目录下）:
    python -m eval.prompt_variants                        # 全部变体跑 golden_sufficiency，不落库
    python -m eval.prompt_variants --variant baseline,v_brief --limit 20   # 选变体 + 限量
    python -m eval.prompt_variants --no-save              # 显式不落库（默认行为，CLI 对齐 golden 系）
    python -m eval.prompt_variants --save                 # 每变体落 eval_runs（eval_type='prompt_variant'）
    python -m eval.prompt_variants --fixture              # 启发式判断器（管线演示，零 LLM 零 DB）
...
from eval.golden_sufficiency import (
    label_str,
