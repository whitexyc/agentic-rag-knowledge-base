...
用法（在 ai_service 目录下）:
    python -m eval.memory_type_dataset                   # clf vs LLM 同集对比 + 落库
    python -m eval.memory_type_dataset --clf-only         # 只跑分类模型（不依赖 LLM）
    python -m eval.memory_type_dataset --fixture          # 关键词启发式（确定性，不依赖模型/LLM）
    python -m eval.memory_type_dataset --no-save          # 纯跑分不写 eval_runs
...
from eval.golden_memory import _dialogue_to_extract_inputs, _fact_match
