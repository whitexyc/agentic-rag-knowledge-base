...
用法（在 ai_service 目录下）:
    python -m eval.build_memory_type_dataset        # 构造 + 校验 + 落盘
    python -m eval.build_memory_type_dataset --dump # 打印分类统计（人工复核用）
...
def validate(samples: list[dict]) -> None:
    ...
    try:
        from eval.memory_type_dataset import MEMORY_TYPE_DATASET
