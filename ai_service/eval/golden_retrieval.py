...
用法（在 ai_service 目录下）:
    python -m eval.golden_retrieval                    # 组合模式（默认 hybrid）
    python -m eval.golden_retrieval --mode fts_only    # 消融：仅全文检索
    python -m eval.golden_retrieval --mode vector_only # 消融：仅向量检索
    python -m eval.golden_retrieval --mode graph_only  # 消融：仅图检索
...
def get_git_commit() -> str:
    ...
    try:
        result = subprocess.run(
            ...
            cwd=Path(__file__).resolve().parents[1],  # 仓库根目录
        )
