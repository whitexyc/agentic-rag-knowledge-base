...
用法（在 ai_service 目录下）:
    python -m eval.compare_nli_models                 # 全量 100 对真实对比
    python -m eval.compare_nli_models --limit 10      # 快速冒烟（验证管线）
    python -m eval.compare_nli_models --smoke         # WP-0 加载验证 + 资源实测（3 参考对 + 25 对批量）
    python -m eval.compare_nli_models --skip-mdeberta / --skip-hhem  # 单侧
...
from eval.compare_factcheck_models import build_pairs, load_hhem, hhem_score
from eval.golden_sufficiency import SUFFICIENCY_DATASET
