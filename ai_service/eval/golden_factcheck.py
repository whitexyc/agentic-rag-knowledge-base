...
用法（在 ai_service 目录下）:
    python -m eval.golden_factcheck                # 真实模式：HHEM 判 50 条 + 落库
    python -m eval.golden_factcheck --fixture      # fixture 模式：关键词启发式，不依赖模型
    python -m eval.golden_factcheck --no-save      # 纯跑分，不写 eval_runs
