...
用法（在 ai_service 目录下）:
    python -m eval.benchmark_rrf_k               # k=20-100 步长 10 + 拐点加密 + 两/三通道归因 + 落库
    python -m eval.benchmark_rrf_k --limit 10    # 冒烟（前 N 题）
    python -m eval.benchmark_rrf_k --no-save     # 不写 eval_runs
    python -m eval.benchmark_rrf_k --top-k 5     # 检索深度（默认 5）
...
指标:
    Hit@5 / Recall@5 / MRR（复用 eval.golden_retrieval.compute_metrics +
...
from eval.golden_retrieval import (_aggregate, compute_metrics, get_git_commit,
