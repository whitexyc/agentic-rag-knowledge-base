...
用法（在 ai_service 目录下）:
    python -m eval.golden_query_rewrite              # 真实模式：LLM 改写 + DB 检索对比 + 落库
    python -m eval.golden_query_rewrite --fixture    # fixture 模式：启发式分诊+改写，不依赖 LLM/DB（管线演示）
    python -m eval.golden_query_rewrite --no-save    # 纯跑分，不写 eval_runs
