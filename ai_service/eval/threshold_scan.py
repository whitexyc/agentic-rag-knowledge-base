...
扫描 ①（L2 触发阈值，t ∈ [0.2, 0.8] 步长 0.05）:
    数据: eval.golden_intent.INTENT_DATASET 100 条 ×
...
数据收集（真实测量，缓存到 .ua/ 便于复跑）:
    python -m eval.threshold_scan --collect-l2   # 只收集 L2 数据（LLM 100 次调用，~2-4 分钟）
    python -m eval.threshold_scan --skip-l2      # 不跑 L2（零 LLM），只跑闸门扫描
    python -m eval.threshold_scan                # 默认: 无缓存先收集再扫描
    python -m eval.threshold_scan --no-cache     # 不读不写缓存
    python -m eval.threshold_scan --cache PATH   # 自定义缓存路径
    python -m eval.threshold_scan --min-recall 0.9  # 推荐约束: recall ≥ 0.9 中选 F1 最大
...
_DEFAULT_CACHE = Path(__file__).resolve().parents[1] / ".ua" / "m047_threshold_cache.json"
...
def print_report(l2_records: list[dict], l2_skipped: list[dict],
    ...
    if l2_records:
        ...
        print("\n① L2 触发阈值扫描: 无数据（LLM 收集失败/被跳过）。"
              "可用 python -m eval.threshold_scan --collect-l2 收集。")
