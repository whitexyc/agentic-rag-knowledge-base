...
用法（在 ai_service 目录下）:
    python -m eval.train_sufficiency_classifier            # 默认训练并落盘
    python -m eval.train_sufficiency_classifier --no-save  # 只训练评估，不落盘
    python -m eval.train_sufficiency_classifier --model-path models/sufficiency_clf.joblib

训练数据: eval.golden_sufficiency.SUFFICIENCY_DATASET（100 条：充分 50 /
...
DEFAULT_MODEL_PATH = str(
    Path(__file__).resolve().parents[1] / "models" / "sufficiency_clf.joblib"
)
