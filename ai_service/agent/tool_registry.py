...
class AgentTool:
    ...
    Attributes:
        ...
        no_retry: 禁用 073 自动重试（module-084：外部 MCP 副作用工具 True，
            默认 False 内置工具语义逐字不变）
    """
    ...
    def __init__(self, name: str, description: str, args_schema: dict,
        ...
        # module-084：073 自动重试逐工具排除——默认 False（built-in 10 工具
        # 沿用 073 语义逐字不变）；外部 MCP 工具 no_retry=True（副作用不可
        # 自动重放，动态工具名放不进固定 _NO_RETRY_TOOLS 集合）
    ...
    async def _execute(self, args: dict, ctx) -> str:
        ...
        try:
            ...
            if settings.tool_auto_retry and self.name not in _NO_RETRY_TOOLS \
                    and not self.no_retry:  # module-084：外部副作用工具（no_retry=True）不自动重放
...
class ToolRegistry:
    ...
    def register(self, name: str, description: str, args_schema: dict,
                 ...
                 approval: str = "auto",
                 no_retry: bool = False) -> "ToolRegistry":
        """注册一个工具（同名覆盖，便于测试替换）
