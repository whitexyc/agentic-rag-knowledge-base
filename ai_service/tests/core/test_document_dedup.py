...
def test_semantic_duplicate_hit():
    ...
    existing = Document(id=5, title="旧文档",
                        embedding=[1.0, 0.0, 0.0], duplicate_cluster_id=None)
...
def test_semantic_duplicate_uses_existing_cluster():
    ...
    existing = Document(id=9, title="旧文档",
                        embedding=[1.0, 0.0, 0.0], duplicate_cluster_id="C-77")
    sess = FakeSession([existing])
    emb = FakeEmb(vec=[1.0, 0.0, 0.0])
...
def test_semantic_duplicate_miss_below_threshold():
    """余弦=0 < 0.95 → 不命中（None）"""
    existing = Document(id=5, title="无关文档", embedding=[0.0, 1.0, 0.0])
    sess = FakeSession([existing])
    emb = FakeEmb(vec=[1.0, 0.0, 0.0])
...
def test_semantic_duplicate_embedding_failure_fail_open():
    ...
    existing = Document(id=5, title="旧文档", embedding=[1.0, 0.0, 0.0])
    sess = FakeSession([existing])
...
def test_semantic_duplicate_skips_null_embedding():
    ...
    legacy = Document(id=1, title="旧格式", embedding=None)
