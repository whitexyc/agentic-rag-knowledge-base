        ...
        entity.setProjects(List.of(
                project(
                        ...
                        List.of(
                                ...
                                "多层记忆与身份隔离：构建长期/短期/会话三层记忆（LLM 自动抽取 + TTL 滚动 + content_hash 幂等落库），以 JWT 登录身份（user_id）为主隔离键、匿名降级 IP 兜底，双向隔离（记忆不污染知识库、只查本用户记忆）；普通聊天/流式/Agent 全链路接入，会话恢复无重复、异步落库不阻塞回答。",
