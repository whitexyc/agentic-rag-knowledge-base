...
| `specs/adr/0017-agent-evaluation.md` | Agent 级评估体系（📋 已立项 module-066：tool_call_logs 表落库 + 任务级评测集 30-50 条 + 三层指标（Outcome pass^1/pass^3 / Trajectory 工具正确率 / System 成本步数）+ 确定性判定不用 LLM judge） |
| `specs/adr/0018-mcp-integration.md` | MCP 集成（📋 已立项 module-067：ToolRegistry 动态注册 FastMCP + stdio/Streamable HTTP 双传输挂载 FastAPI + 只暴露 6 只读工具 + PW_MCP_TOKEN fail-closed） |
| `specs/module-066-agent-evaluation/task-brief.md` | Agent 级评估执行简报（📋 WP-A tool_call_logs 落库 / WP-B 任务集 / WP-C 评测脚本+agent_eval_runs / WP-D 回归；ADR-0017 落地） |
| `specs/module-067-mcp-integration/task-brief.md` | MCP 集成执行简报（📋 WP-A mcp_server 动态注册 / WP-B HTTP 挂载+token / WP-C 安全边界 / WP-D 回归；ADR-0018 落地） |
