...
| `specs/adr/0018-mcp-integration.md` | MCP 集成（✅ 已实施 module-067：ToolRegistry 10 工具经官方 FastMCP 暴露为标准 MCP Server——6 只读工具显式白名单（READ_ONLY_TOOLS，不按 group 过滤防多暴露 re_search 状态类工具）+ stdio/Streamable HTTP 双传输（挂载 /ai/mcp，streamable_http_path="/" 适配）+ PW_MCP_TOKEN Bearer 认证（lifespan fail-closed 拒绝启动 + 中间件恒 401 双保险）+ 结果截断 2000 + args_schema 动态转参数模型；mcp==1.26.0 版本适配：stateless_http/json_response 构造参数、无 version kwarg、Mount 不转发 lifespan 需手动进任务组、DNS rebinding 保护关闭（0.0.0.0 部署 Host 校验 421）） |
...
| `specs/module-067-mcp-integration/` | MCP 集成执行简报（✅ 已实施 2026-08-17：ai_service/mcp_server.py（READ_ONLY_TOOLS 白名单 + build_server 动态注册 + exec 动态参数模型 + 轻量 ctx + 截断 2000 + stdio 入口）+ main.py 挂载 /ai/mcp + 认证中间件 + lifespan fail-closed + config mcp_token + requirements mcp==1.26.0；真实冒烟：stdio client 6 工具 + search_knowledge 真实检索、HTTP 401/401/200、fail-closed 启动拒绝、mcp dev 可启；全量 pytest 1075 基线 + 27 新增全绿（1 项 scripts/test_models.py 系 module-050 遗留 ERROR 未触碰）；ADR-0018 落地） |
| `specs/module-068-agent-phase-fix/` | Agent 阶段推进死锁修复执行简报（✅ 已实施 2026-08-17：检索命中即切 generation（`_retrieval_hit` 确定性纯函数零 LLM + `advance_phase` 向后兼容扩展 + 3 轮未命中强制切兜底）+ 预算按阶段（检索 ≤3 + 生成 ≤2 总 5 = max_agent_tools 4→5，仅 tool_phase_split=true 生效）+ 066 评测重跑（对比 id=1 首跑如实记录）+ 全量 pytest 1102 基线 + 14 新增全绿（存量 test_tool_phase_split.py 6 处 schema 序列断言按新推进语义更新，plan 矛盾点已记录 §五）） |
...
- **L4 多轮拼接（WP-A）**：`IntentClassifier.predict_proba(query, prev_user_query=None)`——prev 提供时 list 拼接最近一轮 user query 向量（2048 维，训练同构，参考 memory_conflict_clf 两条嵌入先例）。**当前落盘 intent_clf.joblib 单 query 1024 维训练 → `PW_INTENT_CLASSIFIER_MULTI_TURN` 默认 false，置 true 需先重训配对样本**；未重训置 true 维度不匹配 → router 捕获回退 LLM（fail-open 零回归）。**（2026-08-16 架构评估追加：该多轮拼接已降级不做——能力已被 LLM 路径覆盖（省略句 12/12 全对）+ WP-B 规则层兜底，分类器拼接性价比不足：省略句占比未数据化、线性模型拼向量学不到指代关系、配对样本稀缺（golden_multi_turn 仅 12 对）；代码 fail-open 保留不删，开关恒 false，详见 METRICS 待办区 #8）**
...
## Agent 阶段推进修复（2026-08-17 module-068 追加，只增不删）

- **推进规则变更（重要语义修订）**：module-058 的推进规则是"本轮调用过 generate_answer/verify_answer → 下一轮切 generation"——066 实测暴露此规则在 tool_phase_split=true 下是**死锁**（检索阶段 schema 不含生成工具 → LLM 永远调不到 → 永不切 → 4 轮全检索预算耗尽兜底，pass^1=0.1）。module-068 改为**三条件任一即切 generation**（单向前进不回退）：① 原条件（本轮调过生成工具，兼容 tool_phase_split=false 路径）② **检索命中**（任一检索工具 ∈ {search_knowledge/search_fts/search_vector/search_graph/extract_entities/recall_memory} 本轮返回非空真实结果——确定性零 LLM 判断，排除"（无检索结果）"/"（无相关历史记忆）"空结果标记非空字符串坑 + extract_entities 解析 JSON entities 非空；**re_search 双组补检工具不参与命中判定**）③ **防空转兜底**（检索阶段轮次 ≥ `agent_retrieval_max_rounds` 默认 3 且始终未命中 → 强制切，参数化 PW_AGENT_RETRIEVAL_MAX_ROUNDS）。实现：`_retrieval_hit(name, result)` 纯函数 + `advance_phase(ctx, executed_names, executed_results=None)` 签名向后兼容（results 缺省 None = 旧逻辑）+ 两循环执行处同序收集 results。
...
## 意图路由 Backlog 前三项（2026-08-19 module-072 追加，只增不删，04 文档 #1/#2/#3）

- **#1 上下文改写接入生产（module-072 WP-A）**：eval-only `contextual_rewrite`（golden_multi_turn）迁入 `rag/retrieval/query_rewrite.py` 并入 module-049 分诊式改写链——`llm_rewrite(query, prev=None)` prev 非空走 `_CONTEXTUAL_REWRITE_PROMPT`（"上一轮问题: {prev}" 段），`prepare`/`prepare_query` 加 `history` 参数（`extract_prev` 取最近一条 user 消息）；**触发 = contextual_rewrite_enabled AND history 非空 AND triage(当前句)==vague**（省略句判定零新逻辑：省略句天然无 FTS 术语 → vague → 改写；"那CMS呢"含术语 → precise → 直接检索零 LLM，plan 实现要点 5 已知边界）。**保真锚点 = `f"{prev} {query}"` 拼接双锚**（裸省略句"为什么"3 字作锚无信息量会系统性误杀——实测双锚 cos 0.867 通过 / 裸锚 0.546 被 0.6 拒）。engine.chat prepare 条件 `query_rewrite_enabled or contextual_rewrite_enabled`（OR 独立生效）；_retrieve 加 history 参数 + **缓存 key 附 prev 哈希防同 query 不同 prev 串话题**（contextual 关闭零变化）。
- **#2 WP-D 工具信号接线（module-072 WP-B）**：agent 端点实测**不调 classify**（main.py:809）→ 轨迹走持久化 = engine.py 模块级 `resolve_tool_history(identity)`（request_logs 按 identity + endpoint IN ('agent','agent-lg') 取最近 trace_id → tool_call_logs 按 trace_id 取工具名列表；2s wait_for + 全异常 fail-open None；SQL 全参数化）+ 三处 classify 调用点传参（engine.chat 两分支 hoist 一次 + main.py chat_stream + graph.py classify_intent 经 RAGState.tool_history 可选字段）；**router.py 零改动**。陈旧信号风险被"短句 + 无特征才生效"天然收敛。注意：request_logs 落库是端点 finally fire-and-forget——信号服务"下一轮请求"（真实场景天然满足，E2E 需轮询）。
- **#3 改写喂路由开关评估（module-072 WP-C，四跑实测达标双开）**：golden_intent + golden_multi_turn × query_rewrite off/on 四跑（eval_runs id=53/54/55/56，record_eval_run 快照补 query_rewrite_enabled/contextual_rewrite_enabled 两键）：golden_intent Accuracy on 1.0000 ≥ off 1.0000 − 0.01 + **短路触发 50/100 判对率 100%**（50 题知识题全短路零误杀，30 casual + 20 realtime 全被规则/无术语挡在短路外）+ golden_multi_turn 意图保持 1.0000/1.0000 + 检索 +0.6000/+0.6000 → **query_rewrite_enabled 默认改 true**。contextual 独立决策：意图 12/12 = 接入前不降 + 检索 +0.60 ≥ 接入前 +0.4363 + vague 句改写能力 0/1→1/1（"为什么"检索 0.00→0.60 实测修复）→ **contextual_rewrite_enabled 默认改 true**；self_contained 0.0833 下降系 triage-precise 度量口径（11/12 含术语自包含句不改写，plan 已预言），逐类分解无生产回归（差异声明见 changelog）。两开关独立评测独立决策，独立生效（短路只随 query_rewrite_enabled，contextual-only 不扩散）。
- **真实 E2E（2026-08-19）**：WP-A 两轮 chat round2 "为什么" → 改写"为什么需要Java线程池？" → 5 sources 命中 prev 主题（首跑遇 deepseek 10s 改写超时 fail-open 回退原 query 检索落空=接入前行为，重试成功——外部抖动如实标注）；WP-B 真实 agent 轮（search_knowledge ×3 + search_fts ×1 落库 tool_call_logs）→ resolve_tool_history 查到 → classify("为什么") reason 含"工具历史信号" → knowledge + engine.chat 全链路。

## 工具治理：防重复 + 失败重试 + 日志隐私（2026-08-19 module-073 追加，只增不删）

- **note_to_self 防重复（WP-A）**：`ReactContext.add_note` 改**返回 bool**（True=新增/False=重复已存在）+ 完全一致去重（strip 后逐字比较，**不做近似去重**——scratchpad 重复来自 LLM 同参数机械重复调用，措辞变体是正常产出不应拦截，判定器确定性优先）；`_note_to_self` 重复时返回"笔记已存在（未重复记录）"，比较点取截断 500 后的值（两次相同超长 note 截断结果一致仍判重复）。
- **re_search 同改写 query 守卫（WP-A）**：`ReactContext.last_research_query` 字段 + `_re_search` 在 check_sufficiency **之后**、retrieve **之前**拦截（`rewritten == last_research_query` → "已按该改写重检过，无新结果"）；sufficient 分支提前返回**不更新**守卫字段；空改写（rewritten_query 缺失/等于原 query）同输入二次调用也拦截（防 LLM 拿同一原 query 反复 re_search 空转）。**边界如实标注**：拦截在 check_sufficiency 之后——第二次调用仍会重新评估充分性（ctx.docs 可能已增长），拦的是"重检索 + 文档格式化"大头；**不做输入 query 级预拦截（完全免 LLM）**——文档变化后同 query 合法重评会被误拦。add_docs 累积幂等（_seen_ids）逐字不动。
...
- **分阶段 token 口径**：`_usage_delta` 用"调用前后 usage 列表长度差"把 token 精确归属到每次 LLM 调用（依据：`_record_usage` 在客户端内部逐次触发、顺序与调用严格一致）；`_TOOL_STACK`（工具名栈）提供 `tool` 字段；`stage_tokens()` 按 环路级/工具内 + 按工具名 聚合。**实测分布**：环路级占 **76–80%**、工具内占 **20–24%**；`search_knowledge` 调用最多（39 次）但 token 近零（**纯检索不调 LLM**），`generate_answer` + `re_search` 占工具内 token **90%+**。
- **批次间波动（今天的第二课）**：同配置（同任务/同 seed/同模型）连跑两次，P95 比值 **0.6290 → 0.9179（差 46%）**、框架 pass^1 0.75 → 0.6667。**`--repeat M` 只消除轮内波动，不消除批次间波动**。口径要求：**采样层级必须与结论强度匹配**——推翻 ADR 级结论需**多批次 × 多轮**，单批次 3 轮不足（与 091「单轮采样」同源，但层次更高一层）。
...
## 上下文工程领域（2026-09-11 讨论 + module-093，ADR-0022）

- **四操作框架（LangChain）**：Write（结构化笔记/记忆）/ Select（检索/JIT）/ Compress（压缩）/ Isolate（隔离）；Anthropic 三原语 = clearing（最安全的压缩，零推理成本）/ compaction（摘要折叠）/ memory（跨会话）。项目对照：Write 已有（note_to_self/recall_memory+三层记忆）、Select 已有（RAG 强项）、缺口在 Compress 与预算治理——module-093 补的就是这两块。
- **context rot / lost in the middle**：窗口越满召回越差（中段 -10~25%），买更大窗口只是给重要信息提供更深的埋葬地点。目标是 Anthropic 说的「最小的高信号 token 集」，不是「最多的上下文」。
- **视图式压缩（D1）**：传给 LLM 的 messages 是压缩副本，本地历史保持完整——原始零丢失、对拍公平、tool_call_logs 天然外置存储（Manus「可恢复压缩」原则）。清掉的不是丢了是挪了。
- **clearing 反直觉实证（ADR-0022）**：Anthropic 称 clearing「最安全」，实测单独用 history tokens **反升 31.6%**——归因「每调用视图膨胀 +36%（失锚→长答案复利）」可证，「re-invoke 循环」假说不可证（孤儿 trace 污染）照实标注。**业界的定性要用自己的场景数据检验**。
- **三判据事前定死（ADR-0022 §4）**：质量 −0.10 / history tokens ×0.70 / LLM 次数 ×1.15，任一不满足该臂不引入。裁定：clearing 不引入；clearing+compaction 引入-**灰度默认关**（质量距红线 0.012、收益裕度 0.91%——余量太薄不默认开）。
- **AST 口径教训（连续两模块）**：Developer 报的 23/6/34 是顶层语句数（len(tree.body)），项目红线口径是 **ast.walk 全语句**（实测 152/24/174）。以后 AST 申报必须注明口径。
- **估算器诚实边界**：CJK 0.8 chars/token（中文场景高估安全侧），ASCII 密集文本**低估**（ratio 0.81）——「宁可高估」非普适，三臂同比不受影响。
- **运维教训**：DB created_at 是 UTC（本机 GMT+8，拿本地时间查「未来区间」恒 0）；后台进程「输出走文件」要验证文件真的在长（0 字节 = stdout 死锁）；活性判据选 tool_call_logs（实时）而非 agent_eval_runs（跑完才落库）。
