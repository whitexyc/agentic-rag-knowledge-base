...
- **Agent 工具化**：10 个工具由模型自主编排调用（检索 / 图谱 / 记忆 / 重检 / 生成 / 逐句验证 / 笔记），**按执行阶段分组暴露**——检索阶段只见检索工具、生成阶段才见生成工具，避免误调用、节省 token；工具调用过程实时展示
...
- **825 项自动化测试**（46 个测试文件）+ 真实环境端到端验证
- **评估闭环**：112 题评测集 + 命中率 / 排序质量指标，每次改动可量化对比（配置快照存档），支持单通道消融
- **可观测性**：每次请求全链路可追踪（各阶段耗时 / token 用量 / 缓存命中），可回答"单问题成本"与 P50/P95 延迟
- **架构决策记录**：12 份关键选型留档（检索 / 分块 / 记忆 / 幻觉检测 / 工具治理等）
