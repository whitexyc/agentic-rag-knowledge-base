...
- [x] 📋 RRF 融合实现 — ⚠️ 不适用：P3 评估后不采纳（RRF 分数量纲与 engine._retrieve min_score=0.6 过滤硬阻塞，见 changelog 设计决策 5）
- [x] 📋 golden_retrieval A/B — ⚠️ 不适用：同上
...
- [x] 🧪 RRF 融合单测（⚠️ 不适用：P3 评估后不采纳，见 changelog 设计决策 5）
...
- [x] 🧪 登录对话 → 高质量记忆多档召回（K=5/3 可达）— 真实 E2E（JWT user_id=9001 + 真实 bge-m3）：5 次同义保存被去重合并为 3 父块（不膨胀），recall 返回 3 条多档（不再恒 K=1）；独立构造 K=5（5 条近义候选均值 0.868>0.85 → 5 条）/ K=3（5 条中质候选均值 0.776∈[0.75,0.85) → 3 条）均真实可达
- [x] 🧪 二次同义对话 → 去重触发不膨胀 — 真实 E2E：同义改写 save status=updated，parents 1→1（条数不涨）；不同事实 status=saved parents 1→2（正常新增）
- [x] 🧪 低分记忆不注入 — 真实 E2E：追加无关记忆（猫咪/爬山）后 recall 主题 query 0 条低分注入
...
- 结论:
  ...
  - [ ] ❌ **不通过**
  - [ ] ⚠️ **有条件通过**
- 备注: Tester 验收 **35 项：32 通过 + 3 不适用（P3 可选未采纳）+ 0 失败**。全量回归 **292 passed / 0 failed**（278 基线 + 14 新增）；记忆单测 95/95（含 14 新增）；身份回归 20/20；下游消费者 engine/stream/session 18/18；py_compile OK。**真实 E2E 通过**（真实 PG + 真实 bge-m3 + 真实 HTTP 端点 + JWT token）：① 登录用户（user_id=9001）5 次同义保存→去重合并为 3 父块（不膨胀），recall 3 条多档（不再恒 K=1）；② K=5 真实可达（5 条近义候选均值 0.868 → 5 条）、K=3 真实可达（5 条中质候选均值 0.776 → 3 条）、低质 avg<0.75 → K=1；③ 同义改写触发去重（updated，条数不涨）、不同事实新增；④ 低分记忆不注入；⑤ 用户隔离（user 9002 召回 0）；⑥ chat_stream relevant==count（MIN_SCORE 失真阈值已移除）。测试数据已清理（memory:% 全库 0 行）。P3 三通道 RRF 经评估不采纳，记录 backlog。
